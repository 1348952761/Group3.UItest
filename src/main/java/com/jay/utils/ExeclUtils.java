...
public class ExeclUtils {
    ...
    protected static Log log = new Log(ExeclUtils.class);
    public static void checkExcelVaild(File file) throws Exception{  
        log.info("读取excel文件："+file.getAbsolutePath());
    	if(!file.exists()){  
        	log.error(file.getName()+"  文件不存在！");
            throw new Exception("文件不存在");  
        }  
        if(!(file.isFile() && (file.getName().endsWith(EXCEL_XLS) || file.getName().endsWith(EXCEL_XLSX)))){  
        	log.error(file.getName()+"  文件不是Excel！");
    ...
    public static Workbook getWorkbook(File file) throws IOException{  
        ...
        if(file.getName().endsWith(EXCEL_XLS)){ 
        	log.info(file.getName()+" 是一个office2003文件");
            wb = new HSSFWorkbook(in);  
        }else if(file.getName().endsWith(EXCEL_XLSX)){  
        	log.info(file.getName()+" 是一个office2007或2010文件");
